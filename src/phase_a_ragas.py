...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    results = []
    if per_q and len(per_q) == len(answers):
        for a, pq in zip(answers, per_q):
            faith = getattr(pq, "faithfulness", 0.0)
            ans_rel = getattr(pq, "answer_relevancy", 0.0)
            ctx_prec = getattr(pq, "context_precision", 0.0)
            ctx_rec = getattr(pq, "context_recall", 0.0)
            results.append(RagasResult(
                ...
                answer_relevancy=float(ans_rel),
                context_precision=float(ctx_prec),
                context_recall=float(ctx_rec),
            ))
    else:
        for a in answers:
            results.append(RagasResult(
                ...
                contexts=a["contexts"],
                ground_truth=a["ground_truth"],
                faithfulness=float(a.get("faithfulness", 0.0)),
                answer_relevancy=float(a.get("answer_relevancy", 0.0)),
                context_precision=float(a.get("context_precision", 0.0)),
                context_recall=float(a.get("context_recall", 0.0)),
            ))
    return results
...
def bottom_10(results: list[RagasResult]) -> list[dict]:
    ...
    for i, r in enumerate(bottom):
        diag, fix = DIAGNOSTIC_TREE.get(r.worst_metric, ("Unknown failure", "Review logs"))
        output.append({
            "rank": i + 1,
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    diag, fix = DIAGNOSTIC_TREE.get(dominant_metric, ("Unknown failure", "Review logs"))
